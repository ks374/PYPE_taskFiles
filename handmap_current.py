...
class _Probe:
    ...
    def color(self):
        try:
            its = self.lumDicts
            valKeys = list(its.keys())

            keys = list(its[valKeys[self.lumVal]].keys())
            if(self.colorn > len(keys)):
                #print "-----------"
                #print keys
                #print self.colorn
                #print keys[self.colorn]
                return (its[valKeys[self.lumVal]][keys[len(keys)]], "")
            else:
                return (its[valKeys[self.lumVal]][keys[self.colorn]], "")
        except:
            return ([0,0,0],"")
    ...
    def nextbglum(self, incr=1):
        #try:
        if(1):
            keys = list(self.lumDicts.keys())
            numDicts = len(keys)
            its = self.lumDicts
            valKeys = list(its.keys())
            numOldLumKeys = len(its[valKeys[self.lumVal]].keys())
    ...
    def nextlum(self, incr=1):
        try:
            ...
            its = self.lumDicts
            colorKeys = list(its[valKeys[self.lumVal]].keys())
            if(self.colorn > len(colorKeys)):
                ...
                #print self.colorn
                its = self.lumDicts
                keys2 = list(its[valKeys[self.lumVal]].keys())
                #print keys2
    ...
    def draw(self):
        ...
        self.app.udpy_note_at_location(self.pp(),self.app.udpy.cwidth-250, int(self.app.udpy.cheight/2) - 150 )   ### added int function. Nov, 2023. Taekjun Kim
