...
def RunSet(app):
        ...
        if (P['occl_fract'] > 0.0):
            app.globals.occlid = range(((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots'])))) + (len(eval(P['occl_num_dots']))) + P['no_occl_reps'])*len(app.globals.sampid)
            app.globals.occlid.sort()
            app.globals.sampid = (((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots'])))) + (len(eval(P['occl_num_dots']))) + P['no_occl_reps'])*app.globals.sampid
        ...
        if P['occl_fract'] > 0.0:
                ...
                for k in range(len(eval(P['occl_line_rots']))):
                        for i in range(len(app.globals.occl_line_widths)):
                                ...
                                app.globals.occl_seeds.append(-1)
                                ...
                                app.globals.occl_seeds[app.globals.occl_count] = occl_seeds.randint(0,maxSeed)
                ...
                while not go_on:
                        ...
                        if c == 'm':
                                if t_loc == 2:
                                        ...
                                        info(app, "Incorrect Match")
                                        con(app, "Incorrect Match", 'red')
                                        result = WRONG_RESP
                                        ...
                                        info(app, "Reaction Time")
                                        con(app, "Reaction Time (%d ms)" % rxtime, 'black')
                                        info(app, "Correct Match")
                                        con(app, "Correct Match", 'blue')
                                        raise NoProblem
                                go_on = 1
                        # if subject answers nonmatch
                        if c == 'n':
                                if t_loc == 1:
                                        rxtime = t.ms()
                                        app.encode_plex(rxtime + app.globals.plexStimIDOffset)
                                        info(app, "Reaction Time")
                                        con(app, "Reaction Time (%d ms)" % rxtime, 'black')
                                        info(app, "Incorrect Non-Match")
                                        con(app, "Incorrect Non-Match", 'red')
                                        result = WRONG_RESP
                                        app.encode_plex(WRONG_RESP)
                                        app.encode(WRONG_RESP)
                                        beep(2000,100)
                                        ...
                                        info(app, "Reaction Time")
                                        con(app, "Reaction Time (%d ms)" % rxtime, 'black')
                                        info(app, "Correct Non-Match")
                                        con(app, "Correct Non-Match", 'blue')
                                        raise NoProblem
                                go_on = 1
                        elif c:
                                con(app, "WRONG KEY", 'red')
                                app.encode_plex(FIX_LOST)
                                app.encode(FIX_LOST) #standard event code
                                info(app, "early break")
                                con(app, "You pressed the wrong button", 'red')
                                ...
                                go_on = 1
                        app.idlefn()
                # If you are here then the trial then something went wrong.  
                raise UserAbort                
...
def main(app):
        ...
        app.params = ParamTable(app.notebook, (
                ...
                ("usebar",        "1",               is_boolean,),
                ("trial_tone",    "0",               is_boolean, "tone at every trial"),
                ("grabbeep",    "0",               is_boolean, "beep at bar grab"),
                ...
                ("numdrops_match",    "10",           is_int, "Number of juice drops"),
                ("numdrops_non_match",    "10",           is_int, "Number of juice drops"),
                ...
                ("secondstim_juice", "1", is_boolean),
                ("stim_juice_time", "300", is_int),
                ("stim_juice_time_min", "280", is_int),
                ("stim_juice_time_max", "320", is_int),
                ("std_stim_juice_time", "20", is_int),
                ("Second_Stim_Prob", "0.6",            is_float, "Probability of second stim reward"),
                ...
                ("istime",    "300",               is_int, "Inter-stimulus time"),
                ...
                ("variable_delaytime", "1",            is_boolean, "If 1 then second stimulus duration is picked from a gaussian distribution"),
                ("mu_delaytime", "30",            is_int, "Mu in a Gaussian Distribution to select random second stim time"),
                ("min_delaytime", "10",            is_int, "Min in a Gaussian Distribution to select random second stim time"),
                ("max_delaytime", "50",            is_int, "Max in a Gaussian Distribution to select random second stim time"),
                ("std_delaytime", "20",            is_int, "Std in a Gaussian Distribution to select random second stim time"),
                ("targ_hold",    "50",        is_int, "Duration to fixate at target"),
                ("min_err",        "0",               is_int),
                ("max_err",        "10000",               is_int),
                ("bg_before",    "(64,48,41)",           is_color),
                ("bg_during",    "(64,48,41)",		is_color),
                ("fixlag",        "50",               is_int),
                ("fixwait",        "100",               is_int),
                ("targ_winsize","80",            is_int,    "Target window size"),
                ("targwait",    "50",            is_int,    "Duration to wait for passing through saccades"),
                ("useLookUpTable", "0", is_int, "1:Use preselected non-match; 0:Use selected non-match"),        
                ...
                ("RFscale",        "1",                is_boolean, "1:Scale stimulus size by RF size; 0:Use SampleSize"), 
                ...
                ("SampleSizeFract", ".8", is_float, "Fraction of RF that the size of the stimulus is suppose to be."),
                ...
                ("stimreps", "10", is_int, "Total number of repetitions per stimulus/response combination"),
                ("no_occl_reps", "4", is_int, "Total number of repetitions for no occluders per condition"),
                ("repblocks", "1", is_int, "Total number of repetitions per stimulus/response combination"),
                ...
                ("SubjectPrefix", "m", is_any, "Animal Prefix to use"),
