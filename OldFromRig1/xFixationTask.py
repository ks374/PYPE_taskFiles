## This is a general fixation task class.
## It generates and presents stimulus objects according to the parameters
## entered in the parameter table self.myTaskParams
##
## Each stimulus s with number i are presented in order from 1 to getNumStim()
## Each sprite withing a stimulus becomes visible according to the value that
## s.getSpriteOnset(i) returns relative to the onset of the of the stimulus.
##
##In order to fully inherit from this class the following methods should be implement/replaced:
...
##
##The following methods can be replaced to add additional functionality
##    def getRFSprite(self) return a sprite to display that represents the RF, mostly for testing
##    def setRecordFile(self,filename) sets the record file for this task to filename
##    def getStimulus(self, index):           #Retrieve stimulus # index - Called during ITI
##    def getNumStim(self):                   #Returns the number of stimuli
...
class xFixationTask:
	...
	sshow = min(params['nstim'], self.getNumStim()-stm_start)
        for i in arange(0,sshow):
            spr = self.getStimulus(stm_start+i)
            stim_arr.append(spr)
