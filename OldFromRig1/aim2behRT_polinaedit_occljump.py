...
def RunSet(app):
		...
		app.globals.speedList = [0,0.5,0.4,0.3,0.2,0.1,0.05] # PZ 12/12/16
        for i in range(len(app.globals.speedList)): # PZ 12/12/16
                app.encode_plex(int(round(app.globals.speedList[i]*pype_plex_code_dict('plexFloatMult') + pype_plex_code_dict('plexFloatMult'))))
                app.encode(int(round(app.globals.speedList[i]*pype_plex_code_dict('plexFloatMult') + pype_plex_code_dict('plexFloatMult'))))
		...
		app.globals.speeds = eval(P['motion_speed']) # PZ 12/12/16
		...
		app.globals.speedorder = [] # PZ 12/12/16
        app.globals.stimorder = []
        a = range(len(app.globals.sampid))
        b = range(len(app.globals.speedid)) # PZ 12/12/16
        while nct < (P['repblocks']):
                if(P['randomize'] == 1):
                        random.shuffle(a)
						random.shuffle(b) # PZ 12/12/16
...
def _RunTrial(app, P):
		...
		speed_loc = app.globals.speedid[location]
        ...
        try:
               ...
               ####Now wait for 2nd istime
				...
				speed = app.globals.speedList[speed_loc] # PZ 12/12/16 - delay in s
				#speedListTemp = [0,1,1,1,1,1]
				#true_speed = speedListTemp[speed_loc]/app.globals.speedList[speed_loc] # PZ 12/12/16 
                app.encode('extra') 
        		app.encode(speed_loc+app.globals.plexStimIDOffset)
                app.encode_plex('extra')
        		app.encode_plex(int(speed_loc+app.globals.plexStimIDOffset))
				go_on = 0
				while not go_on:
					# We are waiting for the eye position to move inside the
					# target or nontarget windows.
					# PZ added lines 1249-1289
					occl_seed = randint(0,20)	
					Randy.seed(occl_seed)
					app.globals.occl_arr_new = []		
					app.globals.occl_arr_new.append(Sprite(app.globals.size, app.globals.size, app.globals.xpos[0],app.globals.ypos[0], fb=app.fb, depth=1, on=0, centerorigin=1))
					app.globals.occl_arr_new[0].fill(P['bg_during']+(0,)) # make sprite transparent
					spacing = app.globals.size/float(sqrt(81))
					i = app.globals.occl_dot_widths.index(app.globals.occl_dot_widths[occl_loc - (len(eval(P['occl_line_rots'])) * len(eval(P['occl_num_lines'])))])
					patrnwidth = round(app.globals.occl_dot_widths[i]*(app.globals.size/10),0)
					randomizer = random
					randomizer.seed(occl_seed)
					w = patrnwidth # linewidth
					a = int(app.globals.occl_num_dots[i])*[1]+(81-int(app.globals.occl_num_dots[i]))*[0]
					randomizer.shuffle(a)
					for j in range(9):
					        ...
					        for k in range(9):
					                ...
					                ypos = -spacing*(sqrt(81)-1)/2.0+spacing*k
					                #random xy jitter
									rndx = int(spacing*(Randy.random()-0.5)/2.0)
					                rndy = int(spacing*(Randy.random()-0.5)/2.0)
					                if(draw == 1 and int(w) != 0):
							        		app.globals.occl_arr_new[0].circlefill((P['occl_clr']+(255,)),int(w),int(xpos+rndx), int(ypos+rndy))
											#count = count + 1
					app.globals.dlist.add(app.globals.occl_arr_new[0])
					app.globals.dlist.update()
				    if app.globals.show_occl[app.globals.trnum] == 1:
				       		app.globals.occl_arr_new[0].on()
					app.globals.dlist.update()
					#time.sleep(speed)	
					app.fb.flip()
					if app.globals.show_occl[app.globals.trnum] == 1:
							app.globals.occl_arr_new[0].off()
					app.globals.dlist.update()
					time.sleep(speed)	
					app.fb.flip()
					#count = count + 1
...
def main(app):
        ...
        app.params = ParamTable(app.notebook, (
				...
				("motion_speed", "[]", is_any, "0->0,1->slowest jump,6->fastest jump"), #PZ 12/12/16 
