...
def RunSet(app):
	...
	if(P.has_key(stimColorName)):
		...
		app.encode(colorTuple[2] + app.globals.plexRotOffset)
	##########################
	# Encode the Color Levels.
	color_levels_list = eval(P['ColorLevels'])
	app.encode_plex(len(color_levels_list)+app.globals.plexStimIDOffset) #numcolors with plexStimIDoffset
	app.encode(len(color_levels_list)+app.globals.plexStimIDOffset)
	for i in range(len(color_levels_list)):
		app.encode_plex(int(round(color_levels_list[i]*pype_plex_code_dict('plexFloatMult') + pype_plex_code_dict('plexFloatMult'))))    
		app.encode(int(round(color_levels_list[i]*pype_plex_code_dict('plexFloatMult') + pype_plex_code_dict('plexFloatMult')))) 
	##########################
	...
	app.globals.cid = [] # this is colorid
	...
	prefColor = P['Pref_color']
	endColor = P['End_color']
	print prefColor[0]
	app.globals.stimcount = 0
	
	app.globals.numColors = len(color_levels_list) #this is to facilitate picking of the target location

	# shape_arr will contain 4*number of color levels possible stimuli, in the following order:
	# preferred shape (prefStims) of all colors at fxation and RF center
	# nonpreferred shape (nonPrefStims) of all colors (nonpref_color) at fix and RF center

	for j in range(len(prefStims)):
		for c in range(len(color_levels_list)):
			# make stim of preferred shape in all colors at fixation spot (A/1)
			r1 = endColor[0] + (prefColor[0]-endColor[0])*color_levels_list[c]
  			g1 = endColor[1] + (prefColor[1]-endColor[1])*color_levels_list[c]
			b1 = endColor[2] + (prefColor[2]-endColor[2])*color_levels_list[c]
			element = prefStims[j]
			rotation = prefRots[j]
			...
			app.globals.shape_arr[app.globals.stimcount].polygon((r1,g1,b1), coords1, width=0)
	...
	for i in range(len(nonPrefStims)):
		for c in range(len(color_levels_list)):
			# make stim of nonpreferred shape in all colors at fixation spot (A/1)
			r1 = endColor[0] + (prefColor[0]-endColor[0])*color_levels_list[c]
  			g1 = endColor[1] + (prefColor[1]-endColor[1])*color_levels_list[c]
			b1 = endColor[2] + (prefColor[2]-endColor[2])*color_levels_list[c]
			# make non preferred shape of non preferred co
			...
			coords1 = transpose(reshape(concatenate([xt2[app.globals.pick_stims[app.globals.stimcount]-1]\
							...
							yt2[app.globals.pick_stims[app.globals.stimcount]-1][0:SV.nvrt[app.globals.pick_stims[app.globals.stimcount]-1]*50]]),\
							(2,50*SV.nvrt[app.globals.pick_stims[app.globals.stimcount]-1])))
			app.globals.shape_arr[app.globals.stimcount].polygon((r1,b1,g1), coords1, width=0)
			app.globals.shape_arr[app.globals.stimcount].rotate(360-rotation)
			app.globals.shape_arr[app.globals.stimcount].off()
			app.globals.stimcount = app.globals.stimcount + 1
	...
	p1 = list(arange(1,4*len(color_levels_list),2))
	p1Rf = list(arange(2,2*len(color_levels_list)+1,2))
	p2Rf = list(arange(2*len(color_levels_list)+2,4*len(color_levels_list)+1,2)) 
	app.globals.stim1list = p1*2 #n*pref, n*npref, n*pref, n*npref; m nm nm m
	app.globals.stim2list = p1Rf + p1Rf + p2Rf + p2Rf

	if show2combos == 0: # All four
		app.globals.caselist = list(arange(1,4*len(color_levels_list)+1))
			
	if show2combos == 1: #only pref stim first
		app.globals.caselist = list(arange(1,len(color_levels_list)+1))+\
				list(arange(2*len(color_levels_list)+1, 3*len(color_levels_list)+1))
	...
	try:
		...
		#This task implements pause (f5) at the trial level
			...
			con(app, "behAA: %.0f%% (%d/%d) (ignoring non-C/W outcomes)" % \
					(AA_beh, app.globals.ncorrAA, app.globals.ntrialsAA), 'black')
			con(app, "behAB: %.0f%% (%d/%d)" % \
					(AB_beh, app.globals.ncorrAB, app.globals.ntrialsAB), 'black')
			con(app, "behBA: %.0f%% (%d/%d)" % \
					(BA_beh, app.globals.ncorrBA, app.globals.ntrialsBA), 'black')
			con(app, "behBB: %.0f%% (%d/%d)" % \
					(BB_beh, app.globals.ncorrBB, app.globals.ntrialsBB), 'black')
...
def _RunTrial(app, P):
	...
	# default correct decision is leftward saccade
	t_loc = 2

	# for A/A and B/B, set correct decision to rightward saccade
	if (currentcase <= app.globals.numColors+1 or currentcase >= 3*app.globals.numColors+1):
		t_loc = 1
	...
	try:
		...
		#figure out if there is a delay to target onset after 2nd stim presentation
		if(P['variable_delaytime'] == 1):
			delaytime = random.gauss(P['mu_delaytime'], P['std_delaytime'])
			if(P['setmindelay']):
				if(delaytime < P['mindelaytime']):
					delaytime = P['mindelaytime']
			if(P['setmaxdelay']):
				if(delaytime > P['maxdelaytime']):
					delaytime = P['maxdelaytime']
		else:
		...
		targwin.draw(color='red') #DVP add 8/18/11
		l = targwin.on(1) #DVP add 8/18/11
		p = nontargwin.on(2) #DVP add 8/18/11

		#moved from below DVP 12/14/11
		...
		while tstim2.ms() < delaytime:
			...
			while not go_on and tstim2.ms() < delaytime:
				...
				app.idlefn()
			go_on2 = 0
			while not go_on2 and tstim2.ms() < delaytime:
				# Now we are waiting for the eye position to move inside either target,
				# nontarget, or stim2window(?)
		...
		#otherwise, proceed with regular monitoring for eye movement and turn on targets

		#i think this timer gets reset just in case, keeping it DVP
		tstim2.reset()


		## Now turn on target window, nontargwin and turn off fixwin

		#targwin.draw(color='red') #DVP comment out 8/18/11
		#l = targwin.on(1) #DVP comment out 8/18/11
		#p = nontargwin.on(2) #DVP comment out 8/18/11
		...
		if (delaytime > 0):
			...
			target2.on()
			t2_on = 1
			# flip frame buffer after turned on shape, both targets
			app.globals.dlist.update()
			app.fb.sync_toggle() # DVP add
			app.fb.flip()
			app.encode_plex('targets_on')
			app.encode('targets_on')
			app.udpy.display(app.globals.dlist)
		...
		app.idlefn()
		#t.reset() #old placement of rxt timer; now rxt monitored from onset of stim2 DVP 8/18/11
		# END ADDITION
		...
		# make sure stimulus and spot are off, uncomment this section if needed
		#app.globals.shape_arr[samp_loc2].off()
		#spot.off() ##DVP
		#if(rfSprite is not None):
			#rfSprite.off()
...
def main(app):
	...
	app.params = ParamTable(app.notebook, (
		...
		("setmindelay", "0",	is_boolean, "If 1 then gaussian distribution will be cut off at set minimum"),
		("mindelaytime", "0",	is_int, "Minimum delay time to target onset"),
		("setmaxdelay", "0",	is_boolean, "If 1 then gaussian distribution will be cut off at set maximum"),
		("maxdelaytime", "0",	is_int, "Maximum delay time to target onset"),
		...
		("End_color", "(255,1,1)",	is_color, "Typically gray at the luminance of the pref color"),
		("ColorLevels", "[]", is_any, "Color levels between pref and end colors; 1 is pref and 0 is end"),
		...
		("Show2Combos",	"0", is_int,	"0: all combinations, 1: Pref as reference only"),
		...
		("TaskName","aim2behrxt", is_any, "TaskName"),
