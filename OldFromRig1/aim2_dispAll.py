...
def _RunTrial(app, P,t):
	...
	try:
		...
		while scount < sshow:
			...
			filepre = "stim"
			filepost = ".tif"
			imageName = "%s%s%s%02d_r%d_o%d_m%d%s" % (dir_name,dir_sep,filepre,\
						stimid[scount],rotid[scount],occlid[scount],modeid[scount],filepost)
			app.fb.snapshot(imageName)
			app.fb.flip()
