	    ...
	    self.SSForegroundsToUse = SSForegroundsToUse
            ...
            for k in arange(0, size(self.perceptShapes,0)):
                ...
                for j in arange(0,size(SSMidgroundStims,0)):
                    ...
                    stimList = self.getSecondShape(modeToEncode, 1 , self.perceptShapes[k], SSMidgroundStims[j],SSForegroundsToUse[j],self.midgroundColor,self.foregroundColor, self.rotVals,ssMidOffsetX,ssMidOffsetY,sMidScale,ssOffsetX, ssOffsetY,ssScale,ssRot,useSMAlone,SMAloneMode,useSSAlone)            
...
##                 ("ExtraForeground_forground", "[(0,0,0)]", is_any, "A list of tuples of midground colors to use for the extra foreground stims"),
        ...
        if(params['DoAmbiguousControl']):
            ...
            ambiOnsetsToUse = eval(params['ambiOnsetsToUse'])
            
            widthsToUse = eval(params['AmbiWidths'])

            heightsToUse = eval(params['AmbiHeights'])
	    
            shapeToUse = params['AmbiShape']
            ...
            for i in arange(0,len(widthsToUse)):
                ...
                for k in arange(0, len(self.perceptShapes)):
                    ...
                    for j in arange(0,len(AmbiStims)):
                        ...
                        for h in arange(0,len(ambiOnsetsToUse)):
                
                            ambi_sp_onset = ambiOnsetsToUse[h] 

                            if(shapeToUse == 0):
			
                                stimList = self.getAmbigiousControl(self.ambiguousControlMode+self.perceptModeOffset[self.perceptShapes[k]],self.complexShapeMode,self.perceptShapes[k],AmbiStims[j],self.midgroundColor,self.foregroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset, shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)

                            else:
			
                                stimList = self.getAmbigiousControl(self.ambiguousControlMode+self.perceptModeOffset[self.perceptShapes[k]],self.complexShapeMode + 4,self.perceptShapes[k],AmbiStims[j],self.midgroundColor,self.foregroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset, shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)
			
                            self.myStims.extend(stimList)
                            
                        ambi_sp_onset = 0
			
                    if(params['PresentSameColorAmbiguous']):

                        if(shapeToUse == 0):
			
                            stimList = self.getAmbigiousControl(self.ambiguousControlMode+self.perceptModeOffset[self.perceptShapes[k]],self.foregroundOnlyMode,self.perceptShapes[k],AmbiStims[0],self.midgroundColor,self.midgroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset, shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)

                        else:
			
                            stimList = self.getAmbigiousControl(self.ambiguousControlMode+self.perceptModeOffset[self.perceptShapes[k]],self.foregroundOnlyMode + 4,self.perceptShapes[k],AmbiStims[0],self.midgroundColor,self.midgroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset, shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)
                ...
                if(params['PresentAmbiguousMidgroundAlone']):
                    ...
                    if(shapeToUse == 0):
                        ...
                        stimList = self.getAmbigiousControl(self.ambiguousControlMode+self.perceptModeOffset[self.perceptShapes[0]],self.midgroundOnlyMode,self.perceptShapes[0],AmbiStims[0],self.midgroundColor,self.foregroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset,shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)
    
                    else:
		    
                        stimList = self.getAmbigiousControl(self.ambiguousControlMode+self.perceptModeOffset[self.perceptShapes[0]],self.midgroundOnlyMode + 4,self.perceptShapes[0],AmbiStims[0],self.midgroundColor,self.foregroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset,shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)
		    
                    self.myStims.extend(stimList)
		    
                if(params['PresentPreferredColorAmbiguousMidgroundAlone']):

                    if(shapeToUse == 0):
		    
                        stimList = self.getAmbigiousControl(self.ambiguousControlMode+self.perceptModeOffset[self.perceptShapes[0]],self.perceptMode,self.perceptShapes[0],AmbiStims[0],self.midgroundColor,self.midgroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset,shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)

                    else:
		    
                        stimList = self.getAmbigiousControl(self.ambiguousControlMode+self.perceptModeOffset[self.perceptShapes[0]],self.perceptMode + 4,self.perceptShapes[0],AmbiStims[0],self.midgroundColor,self.midgroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset,shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)
		    
                    self.myStims.extend(stimList)


	#Mode 22 - AmbiguousTemporal Difference Mode

#        if(params['DoAmbiTemporalDifferenceControl']):
#
#            if(params['AmbiTemporalDifferenceUseSameAsComplex'] == 1):
#
#                ambiTemporalDifferenceStims = complexStims
#
#            else:           
#
#                ambiTemporalDifferenceStims = eval(params['AmbiTemporalDifferenceStimNums'])
#
#            ambiOnsetsToUse = eval(params['ambiOnsetsToUse'])
#
#            for i in arange(0,len(ambiOnsetsToUse)):
#
#                ambi_sp_onset = ambiOnsetsToUse[i] 
#
#                widthsToUse = eval(params['AmbiWidths'])
#
#            	heightsToUse = eval(params['AmbiHeights'])
...
#                	for k in arange(0, len(self.perceptShapes)):
#
#                    		for j in arange(0,len(ambiTemporalDifferenceStims)):
#
#                        		myAmbiOffset = params['AmbiOffset'] * self.radius
#
#                        		bg_color = params['bg_during']
...
#                        		if(shapeToUse == 0):
#			
#						stimList = self.getAmbigiousControlTemporalDifference(self.ambiguousTemporalDifferenceMode+self.perceptModeOffset[self.perceptShapes[k]],self.complexShapeMode,self.perceptShapes[k],ambiTemporalDifferenceStims[j],self.midgroundColor,self.foregroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset, shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)
#
#                        		else:
#			
#						stimList = self.getAmbigiousControlTemporalDifference(self.ambiguousTemporalDifferenceMode+self.perceptModeOffset[self.perceptShapes[k]],self.complexShapeMode + 4,self.perceptShapes[k],ambiTemporalDifferenceStims[j],self.midgroundColor,self.foregroundColor,bg_color,self.rotVals,sp_size,width,height,myAmbiOffset, shapeToUse, ambi_sp_onset, cacheSprite=0,useCache=0)
#			
#					self.myStims.extend(stimList)
					
	#mode 23- Albert Control
	
	if(params['DoAlbertControl']):

        	if(params['AlbertUseSameAsComplex'] == 1):

                	albertStims = complexStims

            	else:           

                	albertStims = eval(params['AlbertStimNums'])

  		for k in arange(0, len(self.perceptShapes)):

               		for j in arange(0,len(albertStims)):

                        	stimList = self.getAlbert(self.albertControlMode+self.perceptModeOffset[self.perceptShapes[k]],self.complexShapeMode,self.perceptShapes[k],albertStims[j],self.midgroundColor,self.foregroundColor,self.rotVals, cacheSprite=0,useCache=0)
    ...
    def createParamTable(self,app):         #The parameters for this class
        ...
        self.myTaskParams = ParamTable(self.myTaskNotebook, (
                ...
                ("SSForegroundsToUse","[16,17]", is_any, "The foreground objects to use as second stimuli -either one or as many as midground stims"),
		...
		("AmbiShape", "0", is_boolean, "Ambiguous control shapes (0:square, 1:ellipse)"),
		...
		("PresentSameColorAmbiguous", "1", is_boolean, "If 1 then present the Same Color Ambiguous Shape"),

                ("PresentAmbiguousMidgroundAlone", "1", is_boolean, "If 1 then present the Ambiguous Midground Rectangle alone"),
		
		("PresentPreferredColorAmbiguousMidgroundAlone", "1", is_boolean, "If 1 then present the Ambiguous Midground Rectangle alone in the preferred color"),
		
		#("Ambiguous Control - Temporal Difference Params", None, None, "Mode 22"),
		#
		#("DoAmbiTemporalDifferenceControl", "0", is_boolean, "If 1 then do ambiguous control with temporal difference."),
		#
		#("AmbiTemporalDifferenceUseSameAsComplex", "1", is_boolean, "If 1 then use the shape listed in ComplexStimNums"),
		#
		#("AmbiTemporalDifferenceStimNums", "[]", is_any, "If AmbiTemporalDifferenceUseSameAsComplex is 0 then use only the complex shapes listed here(1-51)"),
		
           	("ambiOnsetsToUse","[100]", is_any, "The midground will appear this many ms after ambiguous square onset"),
		
		("Albert Control", None, None, "Mode 23"),
		
		("DoAlbertControl", "0", is_boolean, "If 1 then do Albert Control"),
		
		("AlbertUseSameAsComplex", "1", is_boolean, "If 1 then use the shapes listed in ComplexStimNums"),
		
		("AlbertStimNums", "[]", is_any, "If AlbertUseSameAsComplex is 0 then instead use only the complex shapes listed here(1-51)"),
    ...
    def encodeTaskParameters(self,app):     #Encode task Params - Called at the beginning of the task
        ...
        pos = eval(params['ambiOnsetsToUse'])
...
#        return myStims
    ...
    def getSecondShape(self,modeToEncode, submode, mg_shape, fg_shape, sec_shape, mg_color, fg_color, rots,sMidOffsetX,sMidOffsetY,sMidScale,ssOffsetX,ssOffsetY,ssScale,ssRot,useSMAlone,SMAloneMode,useSSAlone,cacheSprite=0, useCache=0):
