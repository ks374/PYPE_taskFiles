...
Its a pattern of line occluders. Occluder #, and width can be varied.
Distance between occluder sprite center and stim sprite center can be varied.
Occluder orientation can be changed. Fraction of trials on which occluder
appears can also be controlled.
...
def RunSet(app):
	...
	pick_stims = arange(36)
	random.shuffle(pick_stims)
	
	#Create the necessary sprites; one sprite per sample object
    app.globals.stim_arr = []
	for i in range(P['num_samp']):
		app.globals.stim_arr.append(Sprite(P['SampleSize']*2, P['SampleSize']*2, app.globals.xpos[0], 0, \
						  fb=app.fb, depth=2, on=0, centerorigin=1))
		# fill the square with bg color
		app.globals.stim_arr[i].fill(P['bg_during'])
		# now add stimulus
		coords1 = transpose(reshape(concatenate([xt2[pick_stims[i]-1][0:SV.nvrt[pick_stims[i]-1]*50],\
			yt2[pick_stims[i]-1][0:SV.nvrt[pick_stims[i]-1]*50]]),(2,50*SV.nvrt[pick_stims[i]-1])))
		app.globals.stim_arr[i].polygon(P['SampleStim1_color'], coords1, width=0)
		app.globals.stim_arr[i].off()
	
	# Create the occluder sprite if necessary
	if P['occl_fract'] > 0.0:
		app.globals.occl = Sprite(P['SampleSize']*2, P['SampleSize']*2, app.globals.xpos[0]+P['occl_offset'],\
		0, fb=app.fb, depth=1, on=0, centerorigin=1)
		app.globals.occl.fill(P['bg_during']+(0,)) # make sprite transparent
		spacing = P['SampleSize']*2/float(P['num_occl'])
		# draw parallel lines
		for i in range(P['num_occl']):
			app.globals.occl.rect(0, -spacing*(P['num_occl']-1)/2.0+spacing*i, P['SampleSize'], P['occl_wid'], \
			P['occl_clr']+(255,))
		app.globals.occl.rotate(360-P['occl_rot'], preserve_size=0)

	#Set up the trial buffers. For each trial, we need to a stimulus number and the target location
	#(1:match,2:non-matc)
	nct = 0
	...
	if P['block_samp'] == 1:
		while nct < P['stimreps']:
			for i in range(P['num_samp']):
				b1 = [2*(i-1),2*i-1]*P['block_reps']
				if(P['randomize'] == 1):
					random.shuffle(b1)
				app.globals.stimorder = app.globals.stimorder+b1
			nct = nct+P['block_reps']

	app.globals.hint = []	
	#This is for random hint duration
	#for i in range(len(app.globals.sampid)*P['stimreps']):
	#	app.globals.hint.append(random.randint(0,P['hint']))

	#This is for fixed hint duration but only on a fraction of trials
	hinttrials = int(P['hint_fract']*len(app.globals.stimorder))
	for i in range(hinttrials):
		app.globals.hint.append(P['hint'])
	for i in range(len(app.globals.stimorder)-hinttrials):
		app.globals.hint.append(0)
	
	random.shuffle(app.globals.hint)
...
def _RunTrial(app, P):
	...
	if app.globals.show_occl[app.globals.trnum] == 1:
		app.globals.dlist.add(app.globals.occl)
	...
	try:
		...
		target1.on()
		con(app, " HINT %d " % (app.globals.hint[app.globals.trnum]), 'black')
...
def main(app):
	...
	app.params = ParamTable(app.notebook, (
		...
		("hint_fract",  "1.0",      is_float, "Fraction of trials on which hint is included"),
		...
		("block_samp", "1",             is_int, "Should the two samples be alternated in blocks?"),
		("block_reps",  "25",            is_int, "Number of reps per block for each stimulus/response pairing"),
		("SampleSize",		"100",			is_int, "Sample stimulus size in pixels"),
		("Occluder parameters", None, None),
		("num_occl",	"1",			is_int, "Number of occluders"),
		("occl_wid",	"3",			is_int, "Pattern width in pixels"),
		("occl_rot",	"90",			is_int, "Occluder orientation"),
		("occl_offset", "50",			is_int, "Occluder offset (relative to stim) in pixels"),
