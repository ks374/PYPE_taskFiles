...
"""
This is a simple saccade task to train Odin on saccades.
...
def RunSet(app):
	...
	app.globals.xpos = [shft8dva, -shft8dva]
	app.globals.ypos = [0, 0]
...
def _RunTrial(app, P):
	...
	# # # # # # # # # # # # # # # # # #
	# First choose the target spot

	app.globals.targetX = random.choice(app.globals.xpos)
	app.globals.targetY = fy
	...
	target1 = Sprite(2*P['targ_size'], 2*P['targ_size'],
					  app.globals.targetX, fy, fb=app.fb, depth=1, on=0, centerorigin=1)
	...
	# Create an instance of the FixWin class (defined in pype.py) that
	# will actually keep track of the eye position for you
	targwin = FixWin(app.globals.targetX, fy, P['targ_winsize'], app)
	...
	try:
		...
		## if we are here the fixation duration is done. Now turn off target window, turn off fixwin
		...
		app.udpy.display(app.globals.dlist)
		# Now we're waiting for the subject to acquire the target spot
        print"target_on %d " % (t.ms())
		...
		while not go_on:
			# We are waiting for the eye position to move inside the
			# target window.  Whether this is the case is one of
			# the things that the FixWin class keeps track of.
			while not targwin.inside() and not TESTING:
				# We use the maxrt limit here
				if P['maxrt'] > 0 and t.ms() > P['maxrt']:
					...
					raise MonkError
				app.idlefn()
			# At this point, the targwin.inside returned 1 (meaning eye
			# is inside window).  Sometimes if the spot has just come
			# on and the subject is in the process of saccading across
			...
			while t2.ms() < P['targwait']:
				if not targwin.inside() and not TESTING:
...
def main(app):
	...
	app.params = ParamTable(app.notebook, (
		...
		("maxrt",       "500",          is_int),
		("targ_size",	"20",			is_int, "size of target"),
		("targ_color", 	"(255,0,0)",	is_color, "Target color"),
