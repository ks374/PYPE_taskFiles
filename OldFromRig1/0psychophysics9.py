...
class xFixationTask:
	...
	try:
		...
		while app.running and ( (2*app.globals.stimCorrect)+1 < self.getNumStim()):
	...
	con(app,"%d stimuli presented, %d stimuli remaining" % (stm_start,(self.getNumStim()/2)-stm_start),"Black")
			...
			while not fixwin.inside() and not TESTING:
				...
				if P['abortafter'] > 0 and t.ms() > P['abortafter']:
					...
					con(app, "no acquisition", 'blue')
                                        #pdb.set_trace()
#                                        app.reward(multiplier=params['rmult'],dobeep=1,dojuice=1)
...
##            }
	...
	numUniqueStims = 2*len(app.globals.stimidlist)
        ...
        #for i in arange(0,params['nRepsPerStim']):
                 ...
                 #shuffle(stimNumbers)
        self.myStimList.extend(stimNumbers)
        self.numStim = self.numStim + (2*len(app.globals.stimidlist))
