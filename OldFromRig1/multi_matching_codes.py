...
Its line or dot occluders. Occluder density can be varied.
Two objects are presented in succession. Match saccade right, non-match left.
Samples are multiple randomly chosen objects. Specify number in num_samp.

Changes here -- output mode numbers
Control which trial to run
Specify stimnums in user menu
photodiode bits
...
def RunSet(app):
	...
	#We changed the size of the stim to 90% of the original to have occluder fit over the stim, since occluder is now constraint to square inside RF. 
	xt1 = (SV.Xarr*app.globals.size*.8/SV.spritesize)+0.5
	yt1 = (SV.Yarr*app.globals.size*.8/SV.spritesize)+0.5
	xt2 = xt1.tolist()
	yt2 = yt1.tolist()
	
	#Randomly pick the appropriate number of stims
	...
	for i in range(P['num_samp']):
		element = random.choice(myList)
		myList.remove(element)
		app.globals.pick_stims.append(SV.stmlist[element])
		app.globals.pick_rots.append(SV.rotlist[element])
		...
		app.globals.stim_arr[i].fill(P['bg_during'])
		# now add stimulus
		coords1 = transpose(reshape(concatenate([xt2[app.globals.pick_stims[i]-1]\
							 [0:SV.nvrt[app.globals.pick_stims[i]-1]*50],\
							yt2[app.globals.pick_stims[i]-1][0:SV.nvrt[app.globals.pick_stims[i]-1]*50]]),\
							(2,50*SV.nvrt[app.globals.pick_stims[i]-1])))
		app.globals.stim_arr[i].polygon(P['SampleStim1_color'], coords1, width=0)
		app.globals.stim_arr[i].rotate(360 - app.globals.pick_rots[i])
		app.globals.stim_arr[i].off()
	...
	if P['occl_fract'] > 0.0:
		...
		for i in range(16):
			...
			app.globals.occl_arr[i].fill(P['bg_during']+(0,)) # make sprite transparent			
			#first line occluders
			if (i > 0) and (i < 13):
				spacing = app.globals.size/sqrt(2)/float(P['num_lines'])
				patrnwidth = 1+(i-1)/4
				w = (spacing/6.0)*patrnwidth # linewidth
				for j in range(P['num_lines']):
					app.globals.occl_arr[i].rect(0, -spacing*(P['num_lines']-1)/2.0+spacing*j, app.globals.size/sqrt(2),\
					w, P['occl_clr']+(255,))
				app.globals.occl_arr[i].rotate(360-((i-1)%4)*45, preserve_size=0)
			elif (i > 12):
				spacing = app.globals.size/sqrt(2)/float(sqrt(P['num_dots']))
				patrnwidth = i - 12
				w = (spacing/6.0)*patrnwidth # linewidth
				#trying pseudorandom dots
				for j in range(int(sqrt(P['num_dots']))):
					xpos = -spacing*(sqrt(P['num_dots'])-1)/2.0+spacing*j
					for k in range(int(sqrt(P['num_dots']))):
						ypos = -spacing*(sqrt(P['num_dots'])-1)/2.0+spacing*k
						#random xy jitter
						rndx = int(spacing*(random.random()-0.5)/2.0)
						rndy = int(spacing*(random.random()-0.5)/2.0)
						app.globals.occl_arr[i].circlefill((P['occl_clr']+(255,)),int(w),\
						int(xpos+rndx), int(ypos+rndy))
						
	#Set up the trial buffers. For each trial, we need to a stimulus number, target location
	#(1:match,2:non-match) and occluder id
	nct = 0
	app.globals.sampid = (list(arange(P['num_samp'])))*2
	app.globals.sampid.sort()
	app.globals.tlocid = [1,2]*P['num_samp']
	app.globals.occlid = [0]*P['num_samp']*2 # this is for no occluders
	...
	if (P['occl_fract'] > 0.0):
		...
		app.globals.sampid2 = []
		app.globals.tlocid2 = []
		if (P['use_line_occl'] == 1):
			#Consider rots and densities
			app.globals.occl1 = eval(P['line_modes'])*len(app.globals.sampid)
			app.globals.occl1.sort()
			app.globals.sampid1 = len(eval(P['line_modes']))*app.globals.sampid
			app.globals.tlocid1 = len(eval(P['line_modes']))*app.globals.tlocid
			
		#dot occluders
		if (P['use_dot_occl'] == 1):
			#Consider all densities
			app.globals.occl2 = eval(P['dot_modes'])*len(app.globals.sampid)
			app.globals.occl2.sort()
			app.globals.sampid2 = len(eval(P['dot_modes']))*app.globals.sampid
			app.globals.tlocid2 = len(eval(P['dot_modes']))*app.globals.tlocid

		app.globals.sampid = app.globals.sampid1+app.globals.sampid2
		app.globals.tlocid = app.globals.tlocid1+app.globals.tlocid2
		app.globals.occlid = app.globals.occl1+app.globals.occl2
...
def _RunTrial(app, P):
	...
	try:
		...
		app.encode('occlshape')
		app.encode(occl_loc+app.globals.plexStimIDOffset)
		app.encode('occlmode')
		app.encode(1+app.globals.plexStimIDOffset)
		...
		#if trial gets upto here then trial counts
...
def main(app):
	...
	app.params = ParamTable(app.notebook, (
		...
		("numdrops_err","2",            is_int, "Number of juice drops on an error"),    
		...
		("use_line_occl", "1",			is_int, "Use line occluders?1:yes; 0:no"),
		("line_modes",	"[]",			is_any, "Low density:1-4 (0,45,90,135 rots); Med:5-8; high:9-12"),
		("num_lines", "5",				is_int, "Number of lines"),
		("use_dot_occl", "1",			is_int, "Use dot occluders?1:yes; 0:no"),
		("dot_modes", "[1]",			is_any, "13,14,15 corresponding to the three densities"),
		("num_dots",  "49",				is_int, "Number of dots (preferably a perfect square"),
