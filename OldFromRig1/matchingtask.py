...
def _RunTrial(app, P):
	...
	#Choose sample stimulus and target location
	samp_loc = random.choice([0,3])
	t_loc = random.choice([1,2])
	...
	try:
		...
		## if we are here the fixation duration is done. Now turn on target window, turn off fixwin
		...
		# Again, a dummy flag to help with task control
		go_on = 0
		t2_on = 0 #this denotes whether target 2 is on or not
		while not go_on:
			# We are waiting for the eye position to move inside the
			...
			while not targwin.inside() and not TESTING:
				...
				if thint.ms()> P['hint'] and t2_on == 0:
			...
			while t2.ms() < P['targwait']:
				# Turn on target 2 after the hint duration
				if thint.ms()> random.randint(0,P['hint']) and t2_on == 0:
					app.globals.stim_arr[3-samp_loc+3-t_loc].on()
					target2.on()
					app.globals.dlist.update()
					app.fb.flip()
		...
		while t.ms() < P['targ_hold']:
			if targwin.broke() and not TESTING:
				app.encode_plex(FIX_LOST)
				info(app, "early break")
				con(app, "early break (%d ms)" % t2.ms(), 'red')
				result = BREAK_FIX
