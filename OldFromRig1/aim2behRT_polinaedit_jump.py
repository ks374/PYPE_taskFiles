...
"""
12/9/2016 PZ: This is a working version of polinas moving stimulus code - the stimulus that jumps to random locations in the receptive field square
...
def RunSet(app):
		...
		# PZ 12/7/2016        
		# PZ 12/9/2016 delay of 0.1 is not really used - its for non-moving case, but having bigger delay makes screen freeze for longer
		app.globals.delayList = [0.1,0.25,0.125,0.08333,0.041667,0.0208333,0.0104167] #[0.1,1,0.5,0.25,0.125,0.0625,0.03125]
		for i in range(len(app.globals.delayList)):
				app.encode_plex(int(round(app.globals.delayList[i]*pype_plex_code_dict('plexFloatMult') + pype_plex_code_dict('plexFloatMult'))))
				app.encode(int(round(app.globals.delayList[i]*pype_plex_code_dict('plexFloatMult') + pype_plex_code_dict('plexFloatMult'))))
		#########################
		...
		#app.globals.speeds = [0,1,2,3,4,5,6] # PZ 12/6/2016 
		app.globals.speeds = eval(P['motion_speed']) # PZ 12/7/2016 

        #now we need one for separate look up tables
        ### If occluders are to be shown then:
		# PZ 12/6/2016
		if (P['occl_fract'] > 0.0):
			app.globals.speedid = (((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots'])))) + (len(eval(P['occl_num_dots']))) + P['no_occl_reps'])*len(app.globals.sampid)*app.globals.speeds #edited for speeds
			app.globals.speedid.sort()
			app.globals.occlid = range(((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots'])))) + (len(eval(P['occl_num_dots']))) + P['no_occl_reps'])*len(app.globals.sampid)#*len(app.globals.speeds) #edited for speeds
			app.globals.occlid.sort()
			app.globals.occlid = len(app.globals.speeds)*app.globals.occlid
			app.globals.sampid = (((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots'])))) + (len(eval(P['occl_num_dots']))) + P['no_occl_reps'])*len(app.globals.speeds)*app.globals.sampid #edited for speeds

        app.globals.steporder = [] # PZ 12/6/2016       
		app.globals.stimorder = []
        a = range(len(app.globals.sampid))
        b = range(len(app.globals.speedid)) # PZ 12/6/2016
        while nct < (P['repblocks']):
                if(P['randomize'] == 1):
                        random.shuffle(a)
						random.shuffle(b) # PZ 12/6/2016
...
def _RunTrial(app, P):
        ...
        if(app.globals.stimorder == []):
                a = range(len(app.globals.sampid))
				b = range(len(app.globals.speedid))
                nct = 0
                while nct < (P['repblocks']):
                        if(P['randomize'] == 1):
                                random.shuffle(a)
                ...
                for k in range(len(eval(P['occl_line_rots']))):
                        for i in range(len(app.globals.occl_line_widths)):
                                ...
                                for j in range(app.globals.occl_num_lines[i]):
                                        app.globals.occl_arr[app.globals.occl_count].rect(0, -spacing*(app.globals.occl_num_lines[i]-1)/2.0+spacing*j, app.globals.size,w, P['occl_clr']+(255,))                    
		...
		step_loc = app.globals.speedid[location]
        ...
        try:
               ...
               ####Now wait for 2nd istime
                ...
                app.encode(SAMPLE_ON)
                app.udpy.display(app.globals.dlist)
				app.globals.movingstim = app.globals.stim_arr[samp_loc2].clone() #PZ 12/6/2016 added lines 1182-1185
				newlist = []
				...
				# PZ 12/6/2016 added lines 1196-1208
				count = 0
				#delayList = [0.03125,0.03125,0.03125,0.03125,0.03125,0.03125,0.03125]
				# stepsize means delay
				# speed 0 -> delay 0, speed 1 -> delay 1, speed 2 -> delay 0.25
				#delayList = [4,1,0.5,0.25,0.125,0.0625,0.03125] # delay between movement steps
				delay = app.globals.delayList[step_loc] #0.05				 
				dist = app.globals.size/3.4
				# PZ 12/7/2016 stepsize list introduced to include non-moving case
				stepsizeList = [0,0.25,0.25,0.25,0.25,0.25,0.25]	
				#stepListToEncode = []						
				stepsize = stepsizeList[step_loc]
                move_speed = stepsize/delay  # actual speed
				# original location:
				#x_original = app.globals.movingstim.x
				#y_original = app.globals.movingstim.y
                #print 'speed: ', move_speed 
				print 'step_loc = ', step_loc	
				#print 'delay list: ', app.globals.delayList
				#print 'speeds used: ', app.globals.speeds			      
                app.encode('extra') 
				...
				go_on = 0
				while not go_on:
					# PZ 12/6/2016 added lines 1259-1285
					newstim = app.globals.movingstim.clone()
					#app.globals.dlist.update()
					#print 'wqrwer'					
					if (delay == 0.1):
							moveto_x = 0
							...
							moveto_x = random.randrange(int(dist))*random.choice([-1,1])
       						moveto_y = random.randrange(int(dist))*random.choice([-1,1])
...
def main(app):
        ...
        app.params = ParamTable(app.notebook, (
                ...
                ("motion_speed","[]",is_any,"0->0Hz, 1->4Hz, 2->8Hz, 3->12Hz, 4->24Hz, 5->48Hz, 6->96Hz"), # PZ 12/7/2016
