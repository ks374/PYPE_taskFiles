...
"""
This task is the extention of multi_matching_codes - included are
...
def RunSet(app):
        ...
        #app.encode('mon_ppd')
        #app.encode_plex(P['mon_ppd'])
        #app.encode(P['mon_ppd'])
        ...
        #We changed the size of the stim to 90% of the original to have occluder fit over the stim, since occluder is now constraint to square inside RF. (Yoshito)
        #Added functionality of making the stim size a percentage of the RF
        ...
        app.globals.ref1nonMatch = []
        app.globals.ref2nonMatch = []
        refStims = eval(P['Ref_Stims'])
        rotation = P['Ref1_rot']
        rotation2 = P['Ref2_rot']
        ...
        ref1nonMatch = eval(P['Ref1_Non_Match'])
        ref2nonMatch = eval(P['Ref2_Non_Match'])
        ref1nonMatchRotations = eval(P['Ref1_Non_Match_Rotations'])
        ref2nonMatchRotations = eval(P['Ref2_Non_Match_Rotations'])
        app.globals.stimcount = 0
        if(useLookUpTable == 1):	
                for j in range(len(refStims)):
                        if(j==0):
                                for i in range(len(app.globals.mylookuptable[0])):
                                        lookup = app.globals.mylookuptable[0]
                                        element = lookup[i]
                                        ...
                                        # now add stimulus try creating via a stimfactory????
                                ...
                                for k in range(len(app.globals.mylookuptable[1])):
                                        lookup = app.globals.mylookuptable[1]
                                        element = lookup[k]
                                        app.globals.pick_stims.append(element)
                                        app.globals.pick_rots.append(rotation2) #append the reference rotation
                                        app.globals.stim_arr.append(Sprite(app.globals.size*2, app.globals.size*2, app.globals.xpos[0],\
                                                   app.globals.ypos[0],fb=app.fb, depth=2, on=0, centerorigin=1))
                                        # fill the square with bg color
                                        app.globals.stim_arr[app.globals.stimcount].fill(P['bg_during'])
                                        ...
                                        coords1 = transpose(reshape(concatenate([xt2[app.globals.pick_stims[app.globals.stimcount]-1]\
                                                ...
                                                (2,50*SV.nvrt[app.globals.pick_stims[app.globals.stimcount]-1])))
                                        app.globals.stim_arr[app.globals.stimcount].polygon(P['SampleStim1_color'], coords1, width=0)
                                        app.globals.stim_arr[app.globals.stimcount].rotate(360-rotation2)
                                        app.globals.stim_arr[app.globals.stimcount].off()
                                        app.globals.stimcount = app.globals.stimcount + 1
        elif(useLookUpTable == 0):
                for j in range(len(refStims)):
                        if(j==0):
                                for i in range(len(ref1nonMatch)):
                                        element = ref1nonMatch[i]
                                        rotation = ref1nonMatchRotations[i]
                                        app.globals.pick_stims.append(element)
                                        app.globals.pick_rots.append(rotation) #append the reference rotation
                                        app.globals.stim_arr.append(Sprite(app.globals.size*2, app.globals.size*2, app.globals.xpos[0],\
                                                   app.globals.ypos[0],fb=app.fb, depth=2, on=0, centerorigin=1))
                                ...
                                for j in range(len(ref2nonMatch)):
                                        element = ref2nonMatch[j]
                                        rotation2 = ref2nonMatchRotations[j]
                                        app.globals.pick_stims.append(element)
                                        app.globals.pick_rots.append(rotation2)
                                        app.globals.stim_arr.append(Sprite(app.globals.size*2, app.globals.size*2, app.globals.xpos[0],\
                                                   app.globals.ypos[0],fb=app.fb, depth=2, on=0, centerorigin=1))
                                        app.globals.stim_arr[app.globals.stimcount].fill(P['bg_during'])
                                        coords1 = transpose(reshape(concatenate([xt2[app.globals.pick_stims[app.globals.stimcount]-1]\
        ...
        for j in range(len(refStims)):
                if(j==0):
                        app.globals.pick_stims.append(refStims[0])
                        app.globals.pick_rots.append(rotation) #append the reference rotation
                        app.globals.stim_arr.append(Sprite(app.globals.size*2, app.globals.size*2, app.globals.xpos[0],\
                        app.globals.ypos[0],fb=app.fb, depth=2, on=0, centerorigin=1))
                        # fill the square with bg color
                        ...
                        app.globals.pick_stims.append(refStims[1])
                        app.globals.pick_rots.append(rotation2) #append the reference rotation
                        app.globals.stim_arr.append(Sprite(app.globals.size*2, app.globals.size*2, app.globals.xpos[0],\
                        app.globals.ypos[0],fb=app.fb, depth=2, on=0, centerorigin=1))
                        # fill the square with bg color
        ...
        for j in range(len(refStims)):
                if(j==0):
                        app.globals.pick_stims.append(refStims[0])
                        app.globals.pick_rots.append(rotation) #append the reference rotation
                        app.globals.stim_arr.append(Sprite(app.globals.size*2, app.globals.size*2, P['Stim2X'],\
                        P['Stim2Y'],fb=app.fb, depth=2, on=0, centerorigin=1))
                        # fill the square with bg color
                        app.globals.stim_arr[app.globals.stimcount].fill(P['bg_during'])
                        # now add stimulus
                        coords1 = transpose(reshape(concatenate([xt2[app.globals.pick_stims[app.globals.stimcount]-1]\
                        ...
                        P['Stim2Y'],fb=app.fb, depth=2, on=0, centerorigin=1))
                        # fill the square with bg color
                        app.globals.stim_arr[app.globals.stimcount+1].fill(P['bg_during'])
                        # now add stimulus
                        coords1 = transpose(reshape(concatenate([xt2[app.globals.pick_stims[app.globals.stimcount+1]-1]\
                                [0:SV.nvrt[app.globals.pick_stims[app.globals.stimcount+1]-1]*50],\
                                yt2[app.globals.pick_stims[app.globals.stimcount+1]-1][0:SV.nvrt[app.globals.pick_stims[app.globals.stimcount+1]-1]*50]]),\
                                (2,50*SV.nvrt[app.globals.pick_stims[app.globals.stimcount+1]-1])))
                        app.globals.stim_arr[app.globals.stimcount+1].polygon(P['SampleStim1_color'], coords1, width=0)
                        app.globals.stim_arr[app.globals.stimcount+1].rotate(360-rotation2)
                        app.globals.stim_arr[app.globals.stimcount+1].off()
        ...
        addRefStims = []
        if(len(refStims) == 1):
                addRefStims = (app.globals.stimcount-len(refStims))*[app.globals.stimcount - len(refStims)]
        elif(len(refStims) == 2):
                addRefStims = ((app.globals.stimcount-len(refStims))/2)*[app.globals.stimcount - len(refStims) + 1, app.globals.stimcount - len(refStims)]
        app.globals.sampid = (list(arange(app.globals.stimcount-len(refStims)))) + addRefStims
        app.globals.sampid.sort()
        #print app.globals.sampid
        ...
        if (P['occl_fract'] > 0.0):
                app.globals.occlid = range((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots']))) + len(eval(P['occl_num_dots'])))*len(app.globals.sampid)
                app.globals.occlid.sort()
                app.globals.sampid = ((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots']))) + len(eval(P['occl_num_dots'])))*app.globals.sampid
        ...
        #encode the fixation location HERE!!!!!!!!!!!!!
        ...
        app.encode_plex(int(P['num_samp']))
        app.encode('fix_x')
        ...
        app.encode('numstim')
        app.encode(int(P['num_samp']))

        # Encode the occluder densities
        num_line_densities = len(eval(P['occl_line_widths']))
        num_dot_densities = len(eval(P['occl_dot_widths']))
        line_widths = eval(P['occl_line_widths'])
        dot_widths = eval(P['occl_dot_widths'])
        total_densities = num_line_densities + num_dot_densities
        ...
        for i in range(num_line_densities):
        ...
        for i in range(num_dot_densities):
...
def _RunTrial(app, P):
        ...
        if P['occl_fract'] > 0.0:
                ...
                app.globals.occl_line_widths = eval(P['occl_line_widths'])
                app.globals.occl_num_lines = eval(P['occl_num_lines'])
                app.globals.occl_line_rots = eval(P['occl_line_rots'])
                app.globals.occl_dot_widths = eval(P['occl_dot_widths'])
                app.globals.occl_num_dots = eval(P['occl_num_dots'])
                for k in range(len(eval(P['occl_line_rots']))):
                        for i in range(len(eval(P['occl_num_lines']))):
                ...
                for i in range(len(eval(P['occl_num_dots']))):
        ...
        #samp_loc2 = app.globals.sampid[app.globals.stimorder[app.globals.trnum]]
        location = app.globals.stimorder.pop(0)
        samp_loc2 = app.globals.sampid[location]
        if(samp_loc2 >= 0):
                if(samp_loc2 < ((app.globals.stimcount - len(refStims))/len(refStims)) or samp_loc2 == app.globals.stimcount - len(refStims)):
                        samp_loc = app.globals.stimcount
                elif(samp_loc2 >= ((app.globals.stimcount - len(refStims))/len(refStims)) and samp_loc2 < (app.globals.stimcount) and samp_loc2 != app.globals.stimcount - len(refStims)):
                        samp_loc = app.globals.stimcount + 1
        app.globals.dlist.add(app.globals.stim_arr[samp_loc]) #sample1	
        app.globals.dlist.add(app.globals.stim_arr[samp_loc2])#sample2
        t_loc = 2
        if(samp_loc - len(refStims) == samp_loc2):
                t_loc = 1
        ...
        try:
                ...
                app.encode_plex(app.globals.pick_stims[samp_loc]+app.globals.plexStimIDOffset)
                app.encode_plex('rotid')
                app.encode_plex(app.globals.pick_rots[samp_loc]+app.globals.plexRotOffset)
                ...
                app.encode_plex(0+app.globals.plexStimIDOffset)
                app.encode('stimid')
                app.encode(app.globals.pick_stims[samp_loc]+app.globals.plexStimIDOffset)
                app.encode('rotid')
                app.encode(app.globals.pick_rots[samp_loc]+app.globals.plexRotOffset)
                ...
                while t.ms() < secondStimTime:
                        #print t.ms()
                        if((t.ms() <= (P['stim_juice_time'] + 4) and t.ms() >= (P['stim_juice_time'] - 4)) and (P['secondstim_juice'] == 1) and juice_done == 0):
...
def main(app):
        ...
        app.params = ParamTable(app.notebook, (
                ...
                ("num_samp",		"2",			is_int, "Number of stimuli; less than or equal to 36"),
                ("useLookUpTable", "1", is_int, "1:Use preselected non-match; 0:Use selected non-match"),		
                ("Ref_Stims", "[]", is_any, "Stim Reference Number"),
                ("Ref1_rot", "0", is_int, "Rotation of Reference 1 Stim"),
                ("Ref1_Non_Match", "[]", is_any, "Non-match stims for Reference 1 Stim"), 
                ("Ref1_Non_Match_Rotations", "[]", is_any, "Non-match stim rotations for Reference 1 Stim"),
                ("Ref2_rot", "0", is_int, "Rotation of Reference 2 Stim"), 
                ("Ref2_Non_Match", "[]", is_any, "Non-match stims for Reference 2 Stim"),
                ("Ref2_Non_Match_Rotations", "[]", is_any, "Non-match stim rotations for Reference 2 Stim"),
                ("StimX",			"0",			is_int, "Stimulus X location"),
                ("StimY",			"0", 			is_int, "Stimulus Y location"),
				("Stim2X",			"0",			is_int, "Stimulus 1 X location"),
                ("Stim2Y",			"0", 			is_int, "Stimulus 1 Y location"),
