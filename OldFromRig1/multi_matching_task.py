...
def RunSet(app):
	...
	nct = 0
	app.globals.sampid = (list(arange(app.globals.stimcount-len(refStims))))*2
	app.globals.sampid.sort()
	app.globals.tlocid = [1,2]*(app.globals.stimcount-len(refStims))
	app.globals.occlid= [0]*(app.globals.stimcount-len(refStims))*2# this is for no occluders
	app.globals.ref_loc = [1,2]*(app.globals.stimcount-len(refStims))
	...
	if (P['occl_fract'] > 0.0):
		...
		app.globals.tlocid = ((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots']))) + len(eval(P['occl_num_dots'])))*app.globals.tlocid
		app.globals.sampid = ((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots']))) + len(eval(P['occl_num_dots'])))*app.globals.sampid
		app.globals.ref_loc = ((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots']))) + len(eval(P['occl_num_dots'])))*app.globals.ref_loc
	...
	occltrials = int(P['occl_fract']*len(app.globals.stimorder))
...
def _RunTrial(app, P):
	...
	if P['occl_fract'] > 0.0:
		...
		for i in range(len(eval(P['occl_num_dots']))):
			...
			app.globals.occl_arr[app.globals.occl_count].fill(P['bg_during']+(0,)) # make sprite transparent
			spacing = app.globals.size/sqrt(2)/float(sqrt(app.globals.occl_num_dots[i]))
			...
			for j in range(int(sqrt(app.globals.occl_num_dots[i]))):
				...
				for k in range(int(sqrt(app.globals.occl_num_dots[i]))):
					ypos = -spacing*(sqrt(app.globals.occl_num_dots[i])-1)/2.0+spacing*k
					#random xy jitter
	...
	con(app,"%d stimuli presented, %d stimuli remaining" % (app.globals.natmpt,len(app.globals.stimorder)-app.globals.natmpt),"Black")
	# # # # # # # # # # # # # # # # # #
	...
	samp_loc = app.globals.stimcount - 1 #default is always reference stim 2
	b = list(arange(1,len(refStims)+1)) # randomly pick which reference stim to use.  Only two options so therefore 50/50 chance.
	ref_loc = random.choice(b)
	if(ref_loc == 1):
		samp_loc = app.globals.stimcount - len(refStims) # Reference stim 1 will be 2 before the end stimcount (last two on stim array are the two reference stims)
	...
	if t_loc == 1:
		...
		if(ref_loc == 1):
			a = list(arange((app.globals.stimcount - len(refStims))/len(refStims)))#create a new list that is for reference stim 1's non-match
		else:
			a = list(arange((app.globals.stimcount - len(refStims))/len(refStims), app.globals.stimcount - len(refStims)))#create a new list that is for reference stim 2's non-match		
		#a.remove(samp_loc)
		samp_loc2 = random.choice(a)
	...
	if app.globals.show_occl[app.globals.trnum] == 1:
		occl_loc = app.globals.occlid[app.globals.stimorder[app.globals.trnum]]
...
def main(app):
	...
	app.params = ParamTable(app.notebook, (
		...
		("redo_breakfix", "0", is_int,  "0:do not repeat breakfix trials, 1:repeat breakfix immediately"),
