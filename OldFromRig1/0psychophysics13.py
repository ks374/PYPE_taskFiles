...
class xFixationTask:
	...
	#print self.myFactory[stm_start]
				...
				#print len(self.myStims)
	#else:
	if(params['RandomReverseOrder']):
			params['ReverseOrder'] = bool(randint(0, 1))
	if(params['ReverseOrder']):
			if(params['MGonly']) or app.globals.gaplist[stm_start] < 0:
...
#			app.fb.flip()
		...
		## If we want to add another ISI at the end of the trial
		    ...
		    match_id = app.globals.matchselect[stm_start]
		    #print gap1
		    print gap2
		    app.encode('test0')
		    app.encode(gap2)
...
################################
	...
	# # # # # # # # # # # # # # # # # #
		...
		if(result != WRONG_RESP):
			...
			app.globals.matchselect.append(app.globals.matchselect[stm_start])
		...
		if((app.globals.stimCorrect+1) > len(app.globals.stimidlist)):
			...
			app.globals.matchselect = []
			app.globals.matchselect.extend(app.globals.matchselectOrig)
			self.myStims = []
			self.diameter = []
...
##            }
	...
	app.globals.matchselect.extend(matchselect)
	print matchselect
	...
	for j in arange(0,len(app.globals.stimidlist)):
	    ...
	    #print matchselect
	    if matchselect[j] == 1:
		if (params['Reference_Mode'] == 4):
			mpicklist_temp = eval(params['Specific_Match_List'])
			for jj in arange(0,len(complexStims)):
				if complexStims[jj] == app.globals.stimidlist[j]:
					mpicklist = mpicklist_temp[jj]
			app.globals.mglist.append(choice(mpicklist))
		else:
		...
		if(params['Reference_Mode'] == 1):
        	...
        	if(params['Reference_Mode'] == 2 ) or (params['Reference_Mode'] == 3):
            		nmpicklist = self.B8ShapeSet
		elif(params['Reference_Mode'] == 4):
			nmpicklist_temp = eval(params['Specific_Non_Match_List'])
			for jj in arange(0,len(complexStims)):
				if complexStims[jj] == app.globals.stimidlist[j]:
					nmpicklist = nmpicklist_temp[jj]
	...
	for j in arange(0,len(app.globals.stimidlist)):
	    ...
	    mixerguy[j].append(app.globals.matchselect[j])
	print mixerguy
	...
	for j in arange(0,len(app.globals.stimidlist)):
	    ...
	    app.globals.matchselect[j] = mixerguy[j][5]
	    print 'test1b'
	    print stupid
	...
	app.globals.matchselectOrig = []
	app.globals.matchselectOrig.extend(app.globals.matchselect)
    ...
    def createParamTable(self,app):         #The parameters for this class
		...
		("Specific_Match_List","[]", is_any, "Match stims for Reference mode 4. Put in as list of lists and number of lists must match number of Ref Stims"),
		#("Specific_Non_Match_List","[]" is_any,"Non Match stims for Reference mode 4. Put in as list of lists and number of lists must match number of Ref Stims"),
		("Specific_Non_Match_List","[]", is_any, "Nonmatch stims for Reference mode 4. Put in as list of lists and number of lists must match number of Ref Stims"),
		("ReverseOrder", 0, is_boolean, "Show complex stim first."),
		("RandomReverseOrder", 0, is_boolean, "Randomizes ReverseOrder."),
