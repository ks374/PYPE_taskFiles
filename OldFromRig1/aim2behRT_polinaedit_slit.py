...
04/04/17: PZ UPD: added reward ramping - borrowed from Dinas task

03/24/17 PZ UPD: allows choosing between motion in random directions or at certain angle 

12/9/2016 PZ: this is the working version of polinas bouncing moving stimulus task

This is a RT version of the match to sample task with occluders.
...
def RunSet(app):
        ...
        densityCheck = 0
        #check if there are any densities over 1.0
        test_line_widths = eval(P['occl_line_widths'])
        test_dot_widths = eval(P['occl_dot_widths'])
        for i in range(len(test_line_widths)):
                if(test_line_widths[i] > 1.0):
                        densityCheck = 1
        for i in range(len(test_dot_widths)):
                if(test_dot_widths[i] > 1.0):
                        densityCheck = 1
        if(densityCheck):
            if ask("pype", "There are densities greater than 1.0, proceed?", ("yes", "no")) == 0:
                pass
            else:
                return
        ...
        app.globals.condition1 = []
        app.globals.condition2 = []
        app.globals.condition3 = []
        app.globals.condition4 = []
		...
		app.globals.slitWidth = [P['slitWidth']]
		app.globals.xposSlit = [P['StimX']-P['slitWidth']*0.5-(app.globals.size-P['slitWidth'])*0.25, P['StimX']-P['slitWidth']*0.5+(app.globals.size-P['slitWidth'])*0.25]
        ...
        app.globals.stepsizeList = [0, 0.25, 0.5, 1, 2, 4, 8]
        for i in range(len(app.globals.stepsizeList)):
                app.encode_plex(int(round(app.globals.stepsizeList[i]*pype_plex_code_dict('plexFloatMult') + pype_plex_code_dict('plexFloatMult'))))
                app.encode(int(round(app.globals.stepsizeList[i]*pype_plex_code_dict('plexFloatMult') + pype_plex_code_dict('plexFloatMult'))))
        #########################
		...
		#app.globals.steps = [0,1,2,3,4,5,6]
		app.globals.steps = eval(P['motion_speed'])
		print 'app.globals.steps = ', app.globals.steps
		#print 'qwerty'

        #now we need one for separate look up tables
        ### If occluders are to be shown then:
        if (P['occl_fract'] > 0.0):
			app.globals.stepid = (((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots'])))) + (len(eval(P['occl_num_dots']))) + P['no_occl_reps'])*len(app.globals.sampid)*app.globals.steps #edited for speeds
			app.globals.stepid.sort()
			app.globals.occlid = range(((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots'])))) + (len(eval(P['occl_num_dots']))) + P['no_occl_reps'])*len(app.globals.sampid)#*len(app.globals.steps) #edited for speeds
			app.globals.occlid.sort()
			app.globals.occlid = len(app.globals.steps)*app.globals.occlid
			app.globals.sampid = (((len(eval(P['occl_num_lines']))* len(eval(P['occl_line_rots'])))) + (len(eval(P['occl_num_dots']))) + P['no_occl_reps'])*len(app.globals.steps)*app.globals.sampid #edited for speeds


		app.globals.steporder = []
		app.globals.stimorder = []
		a = range(len(app.globals.sampid))
		b = range(len(app.globals.stepid))
...
def _RunTrial(app, P):
        ...
        if P['occl_fract'] > 0.0:
				...
				# left half of slit	
				app.globals.occl_arr.append(Sprite((app.globals.size-P['slitWidth'])*0.5, app.globals.size, app.globals.xposSlit[0],app.globals.ypos[0], fb=app.fb, depth=1, on=0, centerorigin=1))
#				app.globals.occl_arr[app.globals.occl_count].fill(P['bg_during']+(0,)) # make sprite transparent
				app.globals.occl_arr[app.globals.occl_count].rect(0, app.globals.xposSlit[0], app.globals.ypos[0], (app.globals.size-P['slitWidth'])*0.5, app.globals.size, P['occl_clr']+(255,))                    
				# right half of slit	
				app.globals.occl_arr.append(Sprite((app.globals.size-P['slitWidth'])*0.5, app.globals.size, app.globals.xposSlit[1],app.globals.ypos[0], fb=app.fb, depth=1, on=0, centerorigin=1))
#				app.globals.occl_arr[app.globals.occl_count].fill(P['bg_during']+(0,)) # make sprite transparent
				app.globals.occl_arr[app.globals.occl_count].rect(0, app.globals.xposSlit[1], app.globals.ypos[0], (app.globals.size-P['slitWidth'])*0.5, app.globals.size, P['occl_clr']+(255,))                    
...
#                app.globals.occl_line_widths = eval(P['occl_line_widths']) + ([0]*P['no_occl_reps'])
#                app.globals.occl_num_lines = eval(P['occl_num_lines']) + ([0]*P['no_occl_reps'])
#                app.globals.occl_line_rots = eval(P['occl_line_rots'])
#                app.globals.occl_dot_widths = eval(P['occl_dot_widths']) + ([0]*P['no_occl_reps'])
#                app.globals.occl_num_dots = eval(P['occl_num_dots']) + ([0]*P['no_occl_reps'])
#                app.globals.occl_seeds = []
#                for k in range(len(eval(P['occl_line_rots']))):
#                        for i in range(len(app.globals.occl_line_widths)):
#                                app.globals.occl_arr.append(Sprite(app.globals.size, app.globals.size, app.globals.xpos[0],\
#                                        app.globals.ypos[0], fb=app.fb, depth=1, on=0, centerorigin=1))
#                                app.globals.occl_seeds.append(0)
#                                app.globals.occl_arr[app.globals.occl_count].fill(P['bg_during']+(0,)) # make sprite transparent
#                                w = round(app.globals.occl_line_widths[i]*(app.globals.size/10),0)
#                                spacing = app.globals.size/float(app.globals.occl_num_lines[i])
...
#                for i in range(len(app.globals.occl_dot_widths)):
                        #for x in range(minSeed,maxSeed+1): # used to make files to test seed
#                        app.globals.occl_arr.append(Sprite(int(app.globals.size), int(app.globals.size), app.globals.xpos[0],app.globals.ypos[0], fb=app.fb, depth=1, on=0, centerorigin=1))
#                        app.globals.occl_seeds.append(0)
#                        app.globals.occl_arr[app.globals.occl_count].fill(P['bg_during']+(0,)) # make sprite transparent
#                        spacing = app.globals.size/float(sqrt(81))
#                        patrnwidth = round(app.globals.occl_dot_widths[i]*(app.globals.size/10),0)
#                        app.globals.occl_seeds[app.globals.occl_count] = occl_seeds.randint(minSeed,maxSeed+1)
                        #app.globals.occl_seeds[app.globals.occl_count] = x #use to make files to test seed comment out line above
...
#                        randomizer.shuffle(a)
                        #file = "grid_locations_seed_%d_density_%d" % (app.globals.occl_seeds[app.globals.occl_count],w)
                        #f = open(file,'w')
                        #f.write(str((a)))
#                        randomizer.seed(app.globals.occl_seeds[app.globals.occl_count])
#                        count = 0
...
#                                        ypos = -spacing*(sqrt(81)-1)/2.0+spacing*k
                                        #random xy jitter
#                                        rndx = int(spacing*(randomizer.random()-0.5)/2.0)
#                                        rndy = int(spacing*(randomizer.random()-0.5)/2.0)
...
#                                                count = count + 1
                                                #use everything below for testing purposes only 
                                                #occluderjitterX.append(rndx)
                                                #occluderjitterY.append(rndy)
                                                #occluderLocationX.append(xpos+rndx)
                                                #occluderLocationY.append(ypos+rndy)
                        #file = "jitterX_seed_%d_density_%d" % (app.globals.occl_seeds[app.globals.occl_count],w)
                        #f = open(file,'w')
                        #f.write(str((occluderjitterX)))
                        #file = "jitterY_seed_%d_density_%d" % (app.globals.occl_seeds[app.globals.occl_count],w)
                        #f = open(file,'w')
                        #f.write(str((occluderjitterY)))
                        #file = "occlLocX_seed_%d_density_%d" % (app.globals.occl_seeds[app.globals.occl_count],w)
                        #f = open(file,'w')
                        #f.write(str((occluderLocationX)))
                        #file = "occlLocY_seed_%d_density_%d" % (app.globals.occl_seeds[app.globals.occl_count],w)
                        #f = open(file,'w')
                        #f.write(str((occluderLocationY)))
...
#                        app.globals.occl_count = app.globals.occl_count + 1
        ...
        try:
               ...
               ####Now wait for 2nd istime
                ...
                # now turn on the stimulus
				secondStimTime = P['maxrt'] #Second stim time set to max RT
				...
				if P['stim2winsz'] == 0:
					rfx,rfy = P['StimX'], P['StimY']
					rfr = ((rfx**2)+(rfy**2))**0.5
					rfz = min_e + (100 - min_e) * rfr / ((app.fb.w+app.fb.h)/2.0)
					# Set a parameter value that's the actual window size to use
					# this trial, so it's saved in data file.
					P['stim2_winsize'] = int(round(P['win_size'] + rfz))
					stim2win = FixWin(P['StimX'], P['StimY'], P['stim2_winsize'], app)
					stim2win.draw(color='green')
					aa = stim2win.on(3)
				else:
					stim2win = FixWin(P['StimX'], P['StimY'], P['stim2winsz'], app)
					stim2win.draw(color='green')
					aa = stim2win.on(3)

				#turn on second stimulus            
#				if(rfSprite is not None):
#                        app.globals.dlist.add(rfSprite)
#                        rfSprite.on()
#                        app.globals.dlist.update()
...
#                app.udpy.display(app.globals.dlist)
				...
				app.globals.dlist.update()
				delaytime = 0
				# set new 'placeholder' timer for 2nd stim presentation
#				tstim2 = Timer ()
#				tstim2.reset()
				#print 'qwerty-4'
				#reset timer to monitor reaction time (measured since 2nd stim onset)
#				t.reset()
				count = 0
				delay = 0.001 # delay between movement steps		
		        app.encode('extra') 
				app.encode(step_loc+app.globals.plexStimIDOffset)
		        app.encode_plex('extra')
				app.encode_plex(int(step_loc+app.globals.plexStimIDOffset))
				#move_dir_flag = P['random_motion_dir']
				stepsize = app.globals.stepsizeList[step_loc]
				if P['random_motion_dir'] == 1:
						move_angle = Randy.random()
						direction_x = randint(1,2)
						direction_y = randint(1,2)
						#print 'dx = ', direction_x, 'dy = ', direction_y
						moveto_x = (math.sqrt(1-move_angle**2))*stepsize*((-1)**direction_x)
						moveto_y = move_angle*stepsize*((-1)**direction_y)
				else:
						param_angle_deg = P['motion_angle']
						#print 'param_angle ', param_angle						
						param_angle = math.radians(param_angle_deg)
						move_angle = math.sin(param_angle)
						#print 'cos(90) = ', math.cos(math.radians(90)) 
						if param_angle < 90:
								moveto_x = (-1)*math.cos(param_angle)*stepsize
								moveto_y = (-1)*math.sin(param_angle)*stepsize
								offset_x = 1
								offset_y = 1
						else:
								moveto_x = (-1)*math.cos(param_angle)*stepsize
								moveto_y = math.sin(param_angle)*stepsize
								offset_x = 2
								offset_y = 1						
						#print 'qwerty222'
				dist = app.globals.size/2.2
				#dist = app.globals.size/(2*math.cos(param_angle))
				move_speed = stepsize/delay # speed of movement, pixels per sec
				...
				if stepsize != 0:
						...
						numsteps = 0	
				if P['random_motion_dir'] == 1:
						moveto_x_total = (math.sqrt(1-move_angle**2))*dist 
						moveto_y_total = move_angle*dist
				else:
						moveto_x_total = math.cos(param_angle)*dist # total dist to go in x dir
						moveto_y_total = math.sin(param_angle)*dist
				#print 'stepsize = ', stepsize, ' ,moveto_x_total = ', moveto_x_total, ' ,moveto_y_total', moveto_y_total
				x_lim_right = app.globals.stim_arr[samp_loc2].x + app.globals.size*0.25
				x_lim_left = app.globals.stim_arr[samp_loc2].x - app.globals.size*0.25
				y_lim_top = app.globals.stim_arr[samp_loc2].y + app.globals.size*0.25
				y_lim_bottom = app.globals.stim_arr[samp_loc2].y - app.globals.size*0.25
				if stepsize != 0:
						if P['random_motion_dir'] == 0:
								app.globals.movingstim.rmove(-0.6*moveto_x_total*((-1)**offset_x),-0.7*moveto_y_total*(-1)**offset_y) 
				...
				# 9/6/2017 PZ - moved here from lines 1216... - target windows were turning on before 2nd stim, fixed
				#turn on target windows at the same time
				targwin.draw(color='red')
				l = targwin.on(1)
				p = nontargwin.on(2)
				...
				# 9/6/2017 PZ - moved timer reset here - was resetting before 2nd stim on
				# set new 'placeholder' timer for 2nd stim presentation
				tstim2 = Timer ()
				tstim2.reset()
				#print 'qwerty-4'
                ...
                app.encode(secondStimTime+app.globals.plexStimIDOffset) #still need to encode this? maybe not DVP ***
				#print 'qwerty-42'
				if not (P['ShowTargWithFix']): 
				...
				while not go_on:
					...
					if P['random_motion_dir'] == 1:
							if count <= numsteps:
									if newstim.x <= x_lim_left or newstim.x >= x_lim_right:
											moveto_x = (-1)*moveto_x
									if newstim.y <= y_lim_bottom or newstim.y >= y_lim_top: 	
											moveto_y = (-1)*moveto_y						
							...
							if (count <= numsteps*2 and count > numsteps):						
									newstim.rmove(-moveto_x,-moveto_y)
							elif count <= numsteps:
									newstim.rmove(moveto_x,moveto_y)
							else:
					...
					newstim.off()														
					count = count + 1	
					#print 'distance_moved_x=',distance_moved_x
					#print 'qwerty-5'					
					if fixwin.broke(0):
						# turn off the 2nd stimulus stimulus
						app.globals.stim_arr[samp_loc2].off()
						spot.off() ##DVP
						if app.globals.show_occl[app.globals.trnum] == 1: #DVPDVP
				...
				while not go_on2:
					while not targwin.inside(1) and not nontargwin.inside(2) and not TESTING:
						...
						if fixwin.broke(0) and stim2win.inside(3):
							app.encode_plex(EARLY_RELEASE)
							app.encode(EARLY_RELEASE)
        ...
        # # # # # # # # # # # # # # # # # #
                ...
                if app.globals.ntrials > -1:
                                ...
                                if((app.globals.ntrials+1) < P['Reward Buffer Size']) :
                                        ...
                                        if attmpt_num2 > 0:
                                                ...
                                                recentbeh2 = 0.0
				#update condition buffers - 04/04/17 ramp reward stuff borrowed from Dina

                if samp_loc2 == 0:
					app.globals.condition1.append(1)
                    if len(app.globals.condition1) > P['Reward Buffer Size']:
                    	app.globals.condition1.pop(0)
					app.globals.condAA.append(1)
					if len(app.globals.condAA) > P['Reward Buffer Size']:
						app.globals.condAA.pop(0)
                if samp_loc2 == 1:
					app.globals.condAA.append(1)
					if len(app.globals.condAA) > P['Reward Buffer Size']:
						app.globals.condAA.pop(0)
                    app.globals.condition2.append(1)
                    if len(app.globals.condition2) > P['Reward Buffer Size']:
                         app.globals.condition2.pop(0)
                if samp_loc2 == 2:
					app.globals.condBB.append(1)
					if len(app.globals.condBB) > P['Reward Buffer Size']:
						app.globals.condBB.pop(0)
                    app.globals.condition3.append(1)
                    if len(app.globals.condition3) > P['Reward Buffer Size']:
                         app.globals.condition3.pop(0)
                if samp_loc2 == 3:
					app.globals.condBB.append(1)
					if len(app.globals.condBB) > P['Reward Buffer Size']:
						app.globals.condBB.pop(0)
                    app.globals.condition4.append(1)
                    if len(app.globals.condition4) > P['Reward Buffer Size']:
                         app.globals.condition4.pop(0)
                condition = [0] * 4
                cond_sum = [0] * 4
                if len(app.globals.condition1) == 0:
                        condition[0] = 0
                else:
                        for i in range(len(app.globals.condition1)):
                                cond_sum[0] = cond_sum[0] + app.globals.condition1[i]
                        condition[0] = 100 * cond_sum[0] / len(app.globals.condition1)
                if len(app.globals.condition2) == 0:
                        condition[1] = 0
                else:
                        for i in range(len(app.globals.condition2)):
                                cond_sum[1] = cond_sum[1] + app.globals.condition2[i]
                        condition[1] = 100 * cond_sum[1] / len(app.globals.condition2)
                if len(app.globals.condition3) == 0:
                        condition[2] = 0
                else:
                        for i in range(len(app.globals.condition3)):
                                cond_sum[2] = cond_sum[2] + app.globals.condition3[i]
                        condition[2] = 100 * cond_sum[2] / len(app.globals.condition3)
                if len(app.globals.condition4) == 0:
                                condition[3] = 0
                else:
                        for i in range(len(app.globals.condition4)):
                                cond_sum[3] = cond_sum[3] + app.globals.condition4[i]
                        condition[3] = 100 * cond_sum[3] / len(app.globals.condition4)
                con(app, "Current condition: %d" % (samp_loc2+1), 'black')
                con(app, "Length of buffers: %d %d %d %d" % (len(app.globals.condition1),len(app.globals.condition2),len(app.globals.condition3),len(app.globals.condition4)), 'black')
                con(app, "Correct in buffer: %d %d %d %d" % (cond_sum[0],cond_sum[1],cond_sum[2],cond_sum[3]), 'black')
                con(app, "Conditions 1:%.0f%% 2:%.0f%% 3:%.0f%% 4:%.0f%%"  % (condition[0],condition[1],condition[2],condition[3]), 'black')
				...
				#mode 2: medium ramping until #trials reaches buffer size, linearly dep. on correct responses after
				if P['reward_mode'] == 2:
					if attmpt_num2 < P['Reward Buffer Size']:
						clk_num = P['numdrops_ramp_b_med']
					else:
						clk_num = P['linear_ramp_drops']*(app.globals.lastX2.count(CORRECT_RESPONSE))
				#mode 3: high%, med%, and low% numdrops based on 94/>75/<75 recbeh
				if P['reward_mode'] == 3:
					if recentbeh2 >= 94:
						clk_num = P['numdrops_ramp_b_high']
					elif (recentbeh2 >= 75 and recentbeh2 < 94):
						clk_num = P['numdrops_ramp_b_med']
					else:
						clk_num = P['numdrops_ramp_b_low']
				#mode 4: calculate recbeh on each condition, reward more if current cond has lowest recbeh
				if P['reward_mode'] == 4:
					condition = [0] * 2
					cond_sum = [0] * 2
					if len(app.globals.condAA) == 0:
						...
						for i in range(len(app.globals.condAA)):
							cond_sum[0] = cond_sum[0] + app.globals.condAA[i]
						condition[0] = 100 * cond_sum[0] / len(app.globals.condAA)
					if len(app.globals.condBB) == 0:
						condition[1] = 0
					else:
						for i in range(len(app.globals.condBB)):
							cond_sum[1] = cond_sum[1] + app.globals.condBB[i]
						condition[1] = 100 * cond_sum[1] / len(app.globals.condBB)
					con(app, "Current condition: %d" % (currentcase), 'black')
					con(app, "Length of buffers (M, NM): %d %d" % (len(app.globals.condAA),len(app.globals.condBB)), 'black')
					con(app, "Correct in buffer: %d %d" % (cond_sum[0],cond_sum[1]), 'black')
					con(app, "Percent in buffer AA:%.0f%% BB:%.0f%%" % (condition[0],condition[1]), 'black')

					#give a bonus for worst performance
					if condition[(currentcase)-1] == min(condition):
						if attmpt_num2 < P['Reward Buffer Size']:
							if recentbeh2 == 94:
								clk_num = P['numdrops_ramp_b_high'] + P['numdrop_bonus']
							elif recentbeh2 > 75:
								clk_num = P['numdrops_ramp_b_med'] + P['numdrop_bonus']
							else:
								clk_num = P['numdrops_ramp_b_low'] + P['numdrop_bonus']
						else:
							clk_num = P['linear_ramp_drops']*(app.globals.lastX2.count(CORRECT_RESPONSE)) + P['numdrop_bonus']

						if clk_num > P['max_drops']:
							clk_num = P['max_drops']

					#otherwise, reward based on overall correct in last X trials of pctbuffer2
					else:
						if attmpt_num2 < P['Reward Buffer Size']:
							if recentbeh2 == 94:
								clk_num = P['numdrops_ramp_b_high']
...
def main(app):
        ...
        app.params = ParamTable(app.notebook, (
				...
				("numdrops_fix",    "0",           is_int, "Number of juice drops"), # PZ 12/7/2016
				("reward_mode", "1",	is_int, "1: Norm, 2: Linear, 3: Hi/Med/Lo%, 4: MinComb + %Comb"), # PZ 04/04/17
				...
				("linear_ramp_drops","2",	is_int, "#drops per correct trial in buffer (mode 2)"), # PZ 04/04/17
				("numdrops_ramp_b_high","23",	is_int, "#drops for 94% perf (mode 3)"), # PZ 04/04/17
				("numdrops_ramp_b_med","18",	is_int, "#drops for >75% perf (mode 3)"), # PZ 04/04/17
				("numdrops_ramp_b_low","13",	is_int, "#drops for <75% perf (mode 3)"), # PZ 04/04/17
				("numdrop_bonus","4",	is_int, "#drops added to numdrops_match for combo w/ lowest% (mode 4) or lower than 75% (mode 5)"), # PZ 04/04/17
				("numdrop_subtraction","2",	is_int, "#drops subtracted for trials other than bonus trials in modes 4 and 5"), # PZ 04/04/17
				("max_drops","22",	is_int, "Maximum #drops (safeguard so bonus/subtraction don't mess up too much if you forgot to set all params)"), # PZ 04/04/17
				("min_drops","10",	is_int, "Minimum #drops (safeguard so bonus/subtraction don't mess up too much if you forgot to set all params)"), # PZ 04/04/17
				...
				("ShowTargWithFix", "0", is_boolean, "if 1 show targets as soon as fixation spot appears"),
                ("delay",    "10",            is_int, "if above is zero then this specifies delay between second stim and target"),
				...
				("motion_speed","[]",is_any,"0->0 mm/s, 1->63.5, 2->127, 3->254, 4->508, 5->1016, 6->2032"), ##PZ    
				("random_motion_dir","0",is_int,"motion in random direction?"), ## 01/26/2017 PZ
				("motion_angle","45",is_float,"set angle (0 to 180 deg) if random_motion_dir = 0"), ## 01/26/2017 PZ
				...
				("slitWidth","50",is_int, "Slit width"),
...
#                ("occl_dot_widths", "[]",         is_any, "Widths of dots."),
                ...
                ("no_occl_reps", "25", is_int, "Total number of repetitions for no occluders per condition"),
