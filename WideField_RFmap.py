# -*- coding: utf-8 -*-
"""
Created on Mon Mar 19, 2025

WideField_RFmap.py is a Fixation task for V4 RF mapping with drifting grating stimuli
...
class WF_RFmap:
    ...
    def __init__(self, app):
        ...
        self.myid = list()
        self.numStim = 0
        self.taskparam = 0   # will be checked in saveEvents
    ...
    def createStimuli(self,app):
        ...
        r_Xmin = params['RangeXmin']
        r_Ymin = params['RangeYmin']        
        r_Xmax = params['RangeXmax']
        r_Ymax = params['RangeYmax']        
        myX, myY = r_Xmin, r_Ymin; 
        ...
        myScale = int(PixPerDeg*2.0);         
        SF = 2; 
        TF = 4; 

        # Make a small vertical drifting square-wave grating
        xwidth = np.linspace(-myScale/2,myScale/2,myScale); 
        ywidth = np.linspace(-myScale/2,myScale/2,myScale);                    					
        [xx, yy] = np.meshgrid(xwidth,ywidth); 

        PixPerCyc = int(PixPerDeg/SF); 
        RadPerPix = (1.0/PixPerCyc) * 2.0*np.pi; 
        Pix_shift_per_frame = PixPerCyc * TF / self.mon_Hz; 

        s = [None]*60; 
        for i in np.arange(60):  # 60 frames = 0.5 sec
            s[i] = Sprite(myScale, myScale, myX, myY, fb=myFB, depth=1, on=0,centerorigin=1)   
            gratingMtx = np.cos(RadPerPix*(xx-Pix_shift_per_frame*i)); 
            mask_pos = gratingMtx > 0
            mask_neg = gratingMtx < 0
            WK_mtx = 172*np.ones((myScale, myScale, 3), dtype=np.uint8);  # RGBA
            WK_mtx[mask_neg] = 0; 
            s[i].alpha_gradient(myScale/2-1, myScale/2, x=0, y=0)		 
            imageNow = np.transpose(WK_mtx,(1,0,2))      
            s[i].array[:] = imageNow.tolist(); 
        self.mySprites.append(s)

        s = [None]*self.Numframes                        
        ...
        stim_table['Xpos_deg'] = np.nan; 
        stim_table['Ypos_deg'] = np.nan;         
        stim_table['SF'] = np.nan;         
        stim_table['TF'] = np.nan;                 

        N_xsteps = r_Xmax - r_Xmin + 1; 
        N_ysteps = r_Ymax - r_Ymin + 1; 
        N_conds = N_xsteps * N_ysteps + 1; 
        stim_table['id'] = np.arange(N_conds); 
        self.myid = np.arange(N_conds); 
        for i in np.arange(N_conds-1):  # last condition is blank
            xpos = i//N_ysteps; 
            ypos = i%N_ysteps; 
            stim_table.at[i,'Xpos_deg'] = r_Xmin + xpos; 
            stim_table.at[i,'Ypos_deg'] = r_Ymin + ypos; 
            stim_table.at[i,'SF'] = 2; 
            stim_table.at[i,'TF'] = 4;         
    ...
    def createParamTable(self,app):
        ...
        self.myTaskParams = ParamTable(self.myTaskNotebook, (        
            ...
            ("RangeXmin", 1, is_int, "Minimum number in X range"),
            ("RangeYmin", -7, is_int, "Minimum number in Y range"),
            ("RangeXmax", 7, is_int, "Maximum number in X range"),
            ("RangeYmax", -1, is_int, "Maximum number in Y range"),
            ...
            ("nstim", "1", is_int, "Number of stimuli"),
            ...
            ("TaskName","WF_RFmap", is_any, "TaskName"),
...
#                self.mySprites[i].__del__()
    ...
    def encodeTaskParameters(self,app):
        ...
        #for Neuropixel, we will not use information below.
        #but, we want to make the same number of fake parameters as we have in the saveEvent
    ...
    def _RunTrial(self,app, t):
        ...
        for i in np.arange(0,sshow):
            ...
            if np.isnan(self.stim_table.at[rowNum,'Xpos_deg']):
                spr = self.mySprites[1]; 
            else:
                myX = self.stim_table.at[rowNum, 'Xpos_deg']*int(gParam['mon_ppd']) + eval(params['FixationShift'])[0]
                myY = self.stim_table.at[rowNum, 'Ypos_deg']*int(gParam['mon_ppd']) + eval(params['FixationShift'])[1]
                spr = []; 
                j = 0; 
                for dir in [0,90,180,270]:
                    for k in np.arange(len(self.mySprites[0])):
                        spr.append(self.mySprites[0][k].clone())
                        spr[j].rotate(-dir); 
                        spr[j].x = myX; 
                        spr[j].y = myY; 
                        j += 1; 
		...
		# I want to know how long it's been since then.)
            ...
            while scount < sshow:
                ...
                app.globals.dlist.add(stim_arr[scount][0:Numframes])
                ...
                stim_arr[scount][foff].off()
                app.globals.dlist.delete(stim_arr[scount][0:Numframes])
                self.toggle_photo_diode(app) #note: toggle_photo_diode updates the dlist
                app.fb.flip()
                print(t.ms(),fon)                 
    ...
    def saveEvents(self,app):# Tomo Neuropixles Mar 8 2019 # edited for Neuropixel 08/08/2020 Taekjun
        ...
        if self.taskparam==0:
            ...
            Header = ['RangeXmin',int(params['RangeXmin']),'RangeYmin',int(params['RangeYmin']),
                      'RangeXmax',int(params['RangeXmax']),'RangeYmax',int(params['RangeYmax']),
                      'stimon',int(params['stimon']),'isi',int(params['IStime']),
                      'numstim',int(params['nstim']),
                      'start_iti']; 
